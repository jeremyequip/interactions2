...
mod_vals <- function(d, modx, modx.values, survey, weights,
  ...
  predefined_args <- c("mean-plus-minus", "plus-minus", "terciles","quartiles")
...
auto_mod_vals <-
  function(d, modx, modx.values, modmean, modsd, modx.labels = NULL,
    ...
    if ((is.null(modx.values) || modx.values == "mean-plus-minus") &
      ...
      group_bound <- paste0("mod", x_or_2,"_bound")
      d[[group_bound]] <- cut2(d[[modx]], g = 3)
      modxvals2 <- as.numeric(levels(d[[group_name]]))
      msg_wrap("Medians of each tercile of ", modx, " are ",
               paste(modxvals2, collapse = ", "))
      
      if (mod2 == FALSE) {
        names(modxvals2) <- c(paste0("Lo ",  levels(d[[group_bound]])[1], " (", modxvals2[1], ")"),
                              paste0("Mid ", levels(d[[group_bound]])[2], " (", modxvals2[2], ")"),
                              paste0("Up ",  levels(d[[group_bound]])[3], " (", modxvals2[3], ")"))
      } else {
    ...
    } else if (!is.null(modx.values) && modx.values[1] == "quartiles") {
      
      x_or_2 <- switch(as.character(mod2),
                       "TRUE" = "2",
                       "FALSE" = "x")
      group_name <- paste0("mod", x_or_2)
      d[[group_name]] <- cut2(d[[modx]], g = 4, levels.median = TRUE)
      group_bound <- paste0("mod", x_or_2,"_bound")
      d[[group_bound]] <- cut2(d[[modx]], g = 4)
      modxvals2 <- as.numeric(levels(d[[group_name]]))
      msg_wrap("Medians of each quartile of ", modx, " are ",
               paste(modxvals2, collapse = ", "))
      
      if (mod2 == FALSE) {
        names(modxvals2) <- c(paste0("Lo ",     levels(d[[group_bound]])[1], " (", modxvals2[1], ")"),
                              paste0("Lo-Mid ", levels(d[[group_bound]])[2], " (", modxvals2[2], ")"),
                              paste0("Up-Mid ", levels(d[[group_bound]])[3], " (", modxvals2[3], ")"),
                              paste0("Up ",     levels(d[[group_bound]])[4], " (", modxvals2[4], ")"))
      } else {
        names(modxvals2) <- c(paste("Lowest quartile of", modx),
                              paste("Lo-Mid quartile of", modx),
                              paste("Up-Mid quartile of", modx),
                              paste("Highest quartile of", modx))
      }
...
values_checks <- function(pred.values = NULL, modx.values, mod2.values) {
  ...
  if (length(modx.values) == 1 &&
      modx.values %nin% c("plus-minus", "terciles", "quartiles")) {
    stop("modx.values must be at least a length of 2.", call. = FALSE)
  }
  if (length(mod2.values) == 1 &&
      mod2.values %nin% c("plus-minus", "terciles", "quartiles")) {
...
split_int_data <- function(d, modx, mod2, linearity.check, modx.values,
  ...
  if (facmod == FALSE &
    ...
    if (!is.null(modx.values) && modx.values[1] == "terciles") {
      ...
      d$modx_group <- factor(cut2(d[[modx]], g = 4, levels.mean = TRUE),
                             labels = c(paste("Lowest quartile of", modx),
                                        paste("Lo-Mid quartile of", modx),
                                        paste("Up-Mid quartile of", modx),
                                        paste("Highest quartile of", modx)))
    }
  ...
  if (!is.null(mod2)) {
    if (facmod2 == FALSE &
      ...
      } else if (!is.null(mod2.values) && mod2.values[1] == "quartiles") {
        d$mod2_group <- factor(cut2(d[[mod2]], g = 4, levels.mean = TRUE),
                               labels = c(paste("Lowest quartile of", mod2),
                                          paste("Lo-Mid quartile of", mod2),
                                          paste("Hi-Mid quartile of", mod2),
                                          paste("Highest quartile of", mod2)))
      }
